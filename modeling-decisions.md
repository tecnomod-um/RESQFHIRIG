# Modeling Decisions - RESQ Stroke Registry Implementation Guide v1.1.0
...
The [RESQ Stroke Discharge Patient Summary Composition](StructureDefinition-resq-stroke-discharge-composition.md) is the document organization layer for the stroke admission. It groups the same profiled resources used throughout the IG into the EHDS-aligned discharge document hierarchy generated by RESQ2FHIR.

This IG intentionally separates two layers:

| | |
| :--- | :--- |
| Clinical resource layer | The profiled`Condition`,`Observation`,`Procedure`,`DiagnosticReport`, medication,`Encounter`,`Patient`,`Organization`and related resources carry the computable clinical facts. |
| Document organization layer | The`Composition`arranges those resources into a discharge document, adds section narratives, and defines the document Bundle entry point. |

The Composition is an index over discrete resources rather than a replacement for them. Narrative text supports human review, while `section.entry` references preserve validation, querying and analytics against the underlying resources.

The section hierarchy follows the Xt-EHR `EHDSDischargeReport` logical model:

`http://www.xt-ehr.eu/fhir/models/StructureDefinition/EHDSDischargeReport`

That alignment maps RES-Q content into the evolving European Hospital Discharge Report structure for EHDS. It is not a claim that this IG is the final normative EU FHIR discharge profile.
